...
void sound_init(void) {
    ...
    static const float lose_arp[] = {392.0f, 311.13f, 233.08f};            // descending
    static const float sel_arp[]  = {659.25f, 987.77f};

    effects[SFX_MOVE]        = make_tone(420.0f, 0.05f, 0.5f, 0.16f);
    effects[SFX_CAPTURE]     = make_arp(cap_arp, 2, 0.04f, 0.5f, 0.20f);
    effects[SFX_KING]        = make_arp(king_arp, 3, 0.05f, 0.5f, 0.24f);
    effects[SFX_WIN]         = make_arp(win_arp, 4, 0.06f, 0.5f, 0.30f);
    effects[SFX_LOSE]        = make_arp(lose_arp, 3, 0.10f, 0.5f, 0.26f);
    effects[SFX_ILLEGAL]     = make_tone(150.0f, 0.10f, 0.5f, 0.20f);
