...
static void poll_keyboard(Input* in) {
    // In the web build's portrait layout the mouse is folded into the touch
    // recognizer instead (poll_touch), so a click is not counted twice.
...
static void poll_touch(Input* in) {
    if (!render_use_portrait()) return; // landscape (desktop-browser) mode: mouse only
    ...
    // Movement tolerance: a third of a square, so a tap that rolls a little
    // still lands, but a deliberate swipe is never read as a tap.
    float slop = (float)render_portrait_square() / 3.0f;
