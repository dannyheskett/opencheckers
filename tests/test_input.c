...
// oc_types.h supplies the types and declares the touch/clock queries, and this
...
// seconds (tap and two-finger durations) and board-square widths (the tap
...
static int     fake_square;    // render_portrait_square() (px per board square)
...
int     render_portrait_square(void) { return fake_square; }

#define DT (1.0 / 60.0)
// 72 px squares => tap slop of 24 px.
#define SQ 72
...
static void test_landscape_ignores_touch(void) {
    ...
    frame_touch(200, 400);
    Input in = frame_release();
    CHECK(!in.touch_tap && !in.menu_up && !in.any_pressed && !in.escape_pressed);
}

// --- The slop scales with the square size ---------------------------------------
static void test_slop_scales_with_square(void) {
    // Big squares (a tablet): 40 px of roll is still a tap at 150 px squares.
    reset();
    fake_square = 150;
    frame_touch(100, 400);
    frame_touch(140, 400);
...
int main(void) {
    ...
    printf("            two-finger menu, swipe menus, mode guards\n");
