...
# OPENCHECKERS_VERSION explicitly; for local `make dist` it derives from the
...
# repo's release.yml is byte-identical. OPENCHECKERS_VERSION still works as an explicit
...
OPENCHECKERS_VERSION ?= $(RELEASE_VERSION)
VERSION_SLUG       := build-$(OPENCHECKERS_VERSION)
...
OUT         := build/opencheckers
OUT_RELEASE := build/opencheckers-release
...
OUT_WIN64 := build/opencheckers-x64.exe
OUT_WIN32 := build/opencheckers-x86.exe
...
OUT_MAC := build/opencheckers-mac
...
# native_app_glue into libopencheckers.so, then package + sign an APK with
...
# where OPENCHECKERS_VERSION is 0 (no release tags yet). versionName is the
# human-facing string. Both are injected at package time (aapt/aapt2 flags), so
# the manifest values are just fallbacks.
ANDROID_VERSION_CODE ?= $(OPENCHECKERS_VERSION)
...
ifeq ($(SIMSTATS),1)
ANDROID_CFLAGS += -DOC_SIMSTATS -DOC_AUTOPLAY
endif
...
# only exists when this flag is present. Without it, dlopen of libopencheckers.so
...
ANDROID_LIB      := $(ANDROID_APK_DIR)/lib/$(ANDROID_ABI)/libopencheckers.so
ANDROID_APK      := build/opencheckers.apk
ANDROID_KEYSTORE ?= build/debug.keystore

# One tiny Java class (OpencheckersActivity, a NativeActivity subclass that hides
...
# mode and NativeActivity loads libopencheckers.so as before.
ANDROID_JAVA_SRC := android/java/com/danheskett/opencheckers/OpencheckersActivity.java
...
# Compile OpencheckersActivity.java against the platform jar, then dex it. d8 emits
...
$(ANDROID_DEX): $(ANDROID_JAVA_SRC)
	...
	$(ANDROID_SDK_BT)/d8 --min-api $(ANDROID_API) --lib $(ANDROID_JAR) \
	    --output build/dex build/java-classes/com/danheskett/opencheckers/*.class
...
$(ANDROID_KEYSTORE):
	...
	keytool -genkeypair -keystore $@ -storepass android -keypass android \
	    -alias opencheckers -keyalg RSA -keysize 2048 -validity 10000 \
	    -dname "CN=opencheckers, O=opencheckers, C=US"
...
$(ANDROID_APK): $(ANDROID_LIB) $(ANDROID_DEX) $(ANDROID_KEYSTORE) \
	...
	$(ANDROID_SDK_BT)/aapt package -f -M android/AndroidManifest.xml \
	    ...
	    -F build/opencheckers.unaligned.apk
	# Store the native lib at lib/<abi>/ inside the APK (path is relative to cwd).
	(cd $(ANDROID_APK_DIR) && $(ANDROID_SDK_BT)/aapt add \
	    ../../build/opencheckers.unaligned.apk lib/$(ANDROID_ABI)/libopencheckers.so)
	# Store classes.dex at the APK root (path relative to cwd = build/dex).
	(cd build/dex && $(ANDROID_SDK_BT)/aapt add \
	    ../opencheckers.unaligned.apk classes.dex)
	$(ANDROID_SDK_BT)/zipalign -f 4 \
	    build/opencheckers.unaligned.apk build/opencheckers.aligned.apk
	$(ANDROID_SDK_BT)/apksigner sign --ks $(ANDROID_KEYSTORE) \
	    --ks-pass pass:android --key-pass pass:android \
	    --out $@ build/opencheckers.aligned.apk
	@rm -f build/opencheckers.unaligned.apk build/opencheckers.aligned.apk
	@echo "[android] built $@"
...
# sideload APK target: same libopencheckers.so, different packaging + a real
...
# ---------------------------------------------------------------------------
ANDROID_AAB        := build/opencheckers.aab
...
$(ANDROID_AAB): $(ANDROID_LIB) $(ANDROID_DEX) $(BUNDLETOOL) $(PLAY_KEYSTORE) \
	...
	cp $(ANDROID_LIB) build/aab/module/lib/$(ANDROID_ABI)/libopencheckers.so
...
# Reuses the mobile touch UI (OC_TOUCH) plus mouse + keyboard, and drives the loop from
...
# build/web/opencheckers.{html,js,wasm} with the mobile shell in web/shell.html.
# ---------------------------------------------------------------------------
...
WEB_OUT     := $(WEB_OUT_DIR)/opencheckers.html
...
web-serve: $(WEB_OUT)
	@echo "Opencheckers: http://localhost:8080/opencheckers.html"
...
# ---------------------------------------------------------------------------
IOS_MIN        ?= 15.0
IOS_APP_NAME   := Opencheckers
IOS_BUNDLE_ID  := com.danheskett.opencheckers
...
IOS_BUILD_NUMBER ?= $(OPENCHECKERS_VERSION)
...
IOS_IPA := build/opencheckers.ipa
...
$(IOS_IPA): $(IOS_DEPS)
	...
	cd build/ios-device && rm -f ../opencheckers.ipa && zip -qr ../opencheckers.ipa Payload
	@if [ -n "$(IOS_SIGN_IDENTITY)" ]; then \
...
dist-ios: $(IOS_IPA)
	@mkdir -p $(DIST)
	cp $(IOS_IPA) $(DIST)/opencheckers-$(VERSION_SLUG)-ios-arm64.ipa
...
#   test_game  — game.c + tick.c (rules, AI, fixed-timestep clock)
#   test_input — input.c's touch tap recognizer, compiled -DPLATFORM_IOS:
...
dist-linux: release
	@rm -rf $(STAGING)/linux && mkdir -p $(STAGING)/linux/opencheckers-$(VERSION_SLUG) $(DIST)
	cp $(OUT_RELEASE) $(STAGING)/linux/opencheckers-$(VERSION_SLUG)/opencheckers
	cp $(DOCS) $(STAGING)/linux/opencheckers-$(VERSION_SLUG)/
	(cd $(STAGING)/linux && tar -czf ../../../$(DIST)/opencheckers-$(VERSION_SLUG)-linux-x86_64.tar.gz opencheckers-$(VERSION_SLUG))

dist-windows: $(OUT_WIN64) $(OUT_WIN32)
	@rm -rf $(STAGING)/win && mkdir -p $(DIST) \
	    $(STAGING)/win/opencheckers-$(VERSION_SLUG)-x64 \
	    $(STAGING)/win/opencheckers-$(VERSION_SLUG)-x86
	cp $(OUT_WIN64) $(STAGING)/win/opencheckers-$(VERSION_SLUG)-x64/opencheckers.exe
	cp $(DOCS) $(STAGING)/win/opencheckers-$(VERSION_SLUG)-x64/
	cp $(OUT_WIN32) $(STAGING)/win/opencheckers-$(VERSION_SLUG)-x86/opencheckers.exe
	cp $(DOCS) $(STAGING)/win/opencheckers-$(VERSION_SLUG)-x86/
	(cd $(STAGING)/win && zip -qr ../../../$(DIST)/opencheckers-$(VERSION_SLUG)-windows-x64.zip opencheckers-$(VERSION_SLUG)-x64)
	(cd $(STAGING)/win && zip -qr ../../../$(DIST)/opencheckers-$(VERSION_SLUG)-windows-x86.zip opencheckers-$(VERSION_SLUG)-x86)

dist-mac: $(OUT_MAC)
	@rm -rf $(STAGING)/mac && mkdir -p $(STAGING)/mac/opencheckers-$(VERSION_SLUG) $(DIST)
	cp $(OUT_MAC) $(STAGING)/mac/opencheckers-$(VERSION_SLUG)/opencheckers
	codesign --force --sign - $(STAGING)/mac/opencheckers-$(VERSION_SLUG)/opencheckers
	cp $(DOCS) $(STAGING)/mac/opencheckers-$(VERSION_SLUG)/
	(cd $(STAGING)/mac && zip -qr ../../../$(DIST)/opencheckers-$(VERSION_SLUG)-macos-universal.zip opencheckers-$(VERSION_SLUG))

# The APK is a self-contained installable, so it ships as-is (versioned name).
dist-android: $(ANDROID_APK)
	@mkdir -p $(DIST)
	cp $(ANDROID_APK) $(DIST)/opencheckers-$(VERSION_SLUG)-android-arm64.apk

# The Play upload artifact: the signed App Bundle (see the android-play target).
dist-android-play: $(ANDROID_AAB)
	@mkdir -p $(DIST)
	cp $(ANDROID_AAB) $(DIST)/opencheckers-$(VERSION_SLUG)-android.aab

# The web build ships as a zip of the HTML/JS/WASM (serve over http to play).
dist-web: $(WEB_OUT)
	@rm -rf $(STAGING)/web && mkdir -p $(STAGING)/web/opencheckers-$(VERSION_SLUG)-web $(DIST)
	cp $(WEB_OUT_DIR)/opencheckers.html $(WEB_OUT_DIR)/opencheckers.js $(WEB_OUT_DIR)/opencheckers.wasm \
	    $(STAGING)/web/opencheckers-$(VERSION_SLUG)-web/
	cp $(DOCS) $(STAGING)/web/opencheckers-$(VERSION_SLUG)-web/
	(cd $(STAGING)/web && zip -qr ../../../$(DIST)/opencheckers-$(VERSION_SLUG)-web-wasm.zip opencheckers-$(VERSION_SLUG)-web)
