...
// window. The display link runs the shared game loop (oc_app_frame); touches and
...
- (void)updateDrawableSize {
    ...
    // clears to the frame's gfx_clear colour (the felt), so it's seamless.
...
- (void)onFrame:(CADisplayLink*)link { (void)link; oc_app_frame(); }
...
- (BOOL)application:(UIApplication*)application
    ...
    vc.view = [[OCMetalView alloc] initWithFrame:bounds];
